...
- **sidepanel.ts** — Glue layer (`mountSidePanel`). Three-region flex layout: `#toolbar` (lifecycle controls + metrics) at the top, scrollable `#events-list` (event feed + sticky new-events chip) in the middle, `#controls` (annotation area) at the bottom. Drives the 4-state `SessionStatus` machine (`idle` / `running` / `paused` / `stopped`) on the panel side, reading `chrome.storage.local` via `onChanged`. Control visibility is **hide-not-disable**: children of `#toolbar` and `#controls` are structurally appended/removed from `buildControlsModel()` — nothing is toggled via `display: none`, so the DoD phrase "absent from the DOM" is pinned by `querySelector === null` tests. **Feature #17 narrows the user-driven verb surface** to a pause-first lifecycle: pre-session shows Start; running shows only Pause; paused shows Resume + Download (when timeline has material events) + Clear (same gate) + End (when a CLI listener is attached). Stop / Discard / Reset are removed from the surface entirely — Clear (`#clear-btn`) reuses the existing destructive confirmation dialog (`#clear-confirm-dialog`) and dispatches the unchanged `DISCARD_SESSION` SW handler; Download (`#download-btn`) opens the existing pre-export reminder and dispatches `STOP_SESSION` + `EXPORT_SESSION`; End (`#end-btn`) dispatches the same `STOP_SESSION` + `EXPORT_SESSION` pair, relying on the existing handoff branch in `service-worker.ts` to route the byte-identical zip to `performHandoff`. The pre-export reminder's confirm button id was renamed `#download-btn` → `#confirm-export-btn` to free `#download-btn` for the toolbar (fixes a latent id collision). End visibility is reactive to listener attach/detach: the panel subscribes to the existing `PENDING_HANDOFF_CHANGED` runtime broadcast and flips a local `listenerAttached` boolean, then re-runs `applyControlsModel()` — no panel re-mount, the `#events-list` node identity is preserved, and `applyControlsModel` saves+restores `document.activeElement.id` across the rebuild so keyboard focus survives a live attach. **Toolbar** hosts: start (pre-session), pause (icon + label swap to Resume in paused state), download / clear / end (paused-state contextual exits), session metrics (with paused badge), empty-state hint. **Controls** hosts: PII mode selector, annotation textarea with embedded element picker icon (inside `.annotation-wrapper`), add-note button, first-run notice, inline pre-export reminder, inline clear-confirmation dialog (danger-tinted, default focus on Cancel, Escape closes), shared `#async-error` line (errors from save/export persist until the next success). All buttons use `<span class="btn-icon">` + `<span class="btn-label">` structure; `withLoadingState(btn, busyLabel, fn)` targets only `.btn-label` to preserve icons during the disabled + `aria-busy="true"` envelope. The standalone screenshot button was removed in feature #12 — the element picker is the only mechanism for attaching a focused (cropped) screenshot to an annotation. **Never imports privileged Chrome capture APIs** (verified by `tests/sidepanel-no-direct-capture.test.ts`); all capture goes through the service worker. The Clear dialog reads event + screenshot counts from a fresh `chrome.storage.local.get` at dialog-open time (not in-memory mirror) so the "Delete N events and M screenshots" count never lies. Cancel on the dialog is a pure UI close — ZERO storage writes, spy-pinned.
...
- **sidepanel-controls.ts** — Pure view-model: `buildControlsModel({status, hasResidualState, hasEvents, listenerAttached})` returns a declarative `ControlVisibility` shape describing which nodes should be mounted in `#toolbar` / `#controls`. Consumed by `sidepanel.ts` for the hide-not-disable structural rendering. Feature #17 widened the inputs with `hasEvents` (does the timeline have any material events — `session_paused` / `session_resumed` markers don't count, see `countMaterialEvents`) and `listenerAttached` (is a CLI handoff config currently set), and reshaped `ControlVisibility` to express the pause-first lifecycle: dropped `stop` / `discard` / `reset`, added `download` / `clear` / `end`. Predicates: `download = clear = (status === "paused" && hasEvents)`; `end = (status === "paused" && listenerAttached)`. The 16-cell matrix (status × hasEvents × listenerAttached) is exhaustively pinned by `sidepanel-controls.test.ts`.
...
| Version | Changes |
|---------|---------|
| (unreleased) | Pause-first lifecycle simplification (feature #17): the user-driven lifecycle verbs narrow from {Start, Pause, Resume, Stop, Discard, Reset} to a smaller, contextual set anchored on Pause-as-primary-action. Pre-session shows only Start. Active (running) sessions show only Pause — there is no Stop or Discard mid-recording, the user must Pause first to ship or clear. Paused sessions show Resume always, plus Download (`#download-btn`) and Clear (`#clear-btn`) when the timeline has at least one material event, plus End (`#end-btn`) when a CLI listener is attached. Stop / Discard / Reset are removed from the surface entirely; Clear subsumes Discard + Reset's roles. The underlying `SessionStatus` machine is unchanged — Clear still calls `DISCARD_SESSION`, Download/End still call `STOP_SESSION` + `EXPORT_SESSION`, and the existing handoff branch in `service-worker.ts` routes End's bytes to `performHandoff` when a config is present. No new SW message types and no `schema_version` change — End is a transport choice over the existing `EXPORT_SESSION` path, byte-identical zip. `ControlsModelInputs` widens with `hasEvents` + `listenerAttached`; `ControlVisibility` drops `stop`/`discard`/`reset` and gains `download`/`clear`/`end`. New `countMaterialEvents` helper filters `session_paused` / `session_resumed` markers so an empty pause shows only Resume (DoD-4). Live listener attach/detach surfaces End reactively: the panel subscribes to the existing `PENDING_HANDOFF_CHANGED` broadcast and flips a local boolean, with `applyControlsModel` saving and restoring `document.activeElement.id` across the rebuild so keyboard focus is preserved. The pre-export reminder's confirm button id was renamed `#download-btn` → `#confirm-export-btn` to free `#download-btn` for the toolbar (fixes a latent id collision). The legacy Discard dialog is renamed to `#clear-confirm-dialog` with the same destructive-confirm semantics (Cancel = zero writes, fresh storage counts at open time, default focus on Cancel, Escape closes). New regression nets: `tests/sidepanel-no-legacy-ids.test.ts` greps `src/`/`tests/`/`e2e/`/`cli/` for `stop-btn`/`discard-btn`/`reset-btn` literals and fails on any orphan; `src/lib/exporter.golden.test.ts` extended with byte-identical Download ≡ End determinism assertion plus a `SCHEMA_VERSION === "1.2.0"` regression pin. |
